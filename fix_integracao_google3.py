# -*- coding: utf-8 -*-
"""Atualiza a etapa Integração com Google Agenda - versão final."""
import sys
sys.path.insert(0, '.')
import sqlite3
import base64
...
content = f"""## Integração com Agendas Externas

### Trazer agenda do Google para o DOit (Pessoal)
...
### Trazer agenda do Google para o DOit (Pública)
...
### Assinar a agenda do DOit em outras agendas (Google, Outlook, etc.)

1. No DOit, localize a agenda que deseja compartilhar
2. Clique com o **botão direito do mouse** no ícone de calendário
3. Selecione **"Copiar endereço do link"**
4. No app de destino (Google Agenda, Outlook, etc.), adicione por URL e cole o link

*Observação: se não houver nenhum evento registrado na agenda, não será possível sincronizar.*

---

*Observações gerais:*
- *O DOit só faz leitura de agendas externas. Ele não edita nem envia eventos de volta.*
- *Trazer para o DOit: somente Google Agenda.*
- *A agenda do DOit pode ser assinada em qualquer outro app que aceite URL iCal (Google, Outlook, Apple Calendar, etc.).*
...
conn.commit()
print("Integração com Agendas Externas atualizada")
conn.close()
